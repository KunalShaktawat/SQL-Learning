...
-- All students(even with no enrollments) left join
select * from students;
select * from enrollments;
select s.student_id,s.student_name,e.enrollment_id,e.course_name 
from students as s
...
-- All enrollments (even if student is missing )
select * from students;
select * from enrollments;
select s.student_id,s.student_name,e.enrollment_id,e.course_name 
from students as s
...
-- Find total credits for each students even not enrolled in any course
...
-- Find students who are enrolled more than two courses
select s.student_id,s.student_name,count(e.course_name)as c from students as s 
join enrollments as e
on s.student_id=e.student_id
group by student_id
having c>2;

-- Total credits earned by students in each major
